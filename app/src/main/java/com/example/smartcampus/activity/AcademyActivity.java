...
public class AcademyActivity extends AppCompatActivity {
    ...
    private TextView et_academy;
    private TextView tv_save;
    private User user;
    private UserCampus userCampus;

    public void initView() {
        et_academy = findViewById(R.id.et_academy);
        ...
        if (userCampus.getAcademy() != null) {
            et_academy.setText(userCampus.getAcademy());
        }
    }
        ...
        tv_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        userCampus.setAcademy(et_academy.getText().toString());
                        Gson gson = new Gson();
                        Connection connection1 = Jsoup.connect(Server.Ip + ":8000/personalDetail");
                        Connection connection = Jsoup.connect(Server.Ip + ":8000/academy");
                        ...
                        try {
                            ...
                            if (result1.isSuccess()) {//获取个人信息成功！
                                ...
                                if (result.isSuccess()) {
                                    //更新本地数据库
                                    userCampusDatabaseHelper.updateAcademy(userDb, userCampus);
                                    finish();
                                } else {
                                    System.out.println(result.getMsg());
                                    Looper.prepare();
                                    Toast.makeText(AcademyActivity.this, result.getMsg(), Toast.LENGTH_SHORT).show();
                                    Looper.loop();
                                }
                            } else {
                            ...
                            e.printStackTrace();
                            Looper.prepare();
                            Toast.makeText(AcademyActivity.this, "网络连接失败！", Toast.LENGTH_SHORT).show();
                            Looper.loop();
                        }
                    }
                }).start();
